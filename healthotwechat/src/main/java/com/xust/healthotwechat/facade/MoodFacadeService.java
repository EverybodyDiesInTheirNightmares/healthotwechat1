...
public class MoodFacadeService {
    ...
    public int entry(MoodForm moodForm){
        ...
        String key = moodForm.getEmail()+"mood";
        try {
            /**用户当天已经录入*/
            ...
            /**redis中加入标记表示三天内录入过*/
            redisTemplate.opsForValue().set(moodForm.getEmail()+"_three_entry_mood",
     ...
     * @return
     */
    public /*List<MoodDto>*/MoodDto findMoodListByOpenid(@Param("email")String email){
        ...
        try {
            ...
            /**查询历史记录*/
            List<Mood> moodList = moodService.findMoodList(email);
            ...
            log.error("心情历史记录查询={}",email+":"+e.getMessage());
            throw e;
        }
