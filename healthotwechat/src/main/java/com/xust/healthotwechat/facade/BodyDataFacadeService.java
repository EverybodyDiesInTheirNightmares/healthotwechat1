...
public class BodyDataFacadeService {
    ...
    public int entry(BodyDataForm bodyDataForm){
      ...
      try {
          String key = bodyDataForm.getEmail()+"bodyData";
          ...
          /**redis中加入标记表示三天内录入过*/
          redisTemplate.opsForValue().set(bodyDataForm.getEmail()+"_three_entry_bodyData",
     ...
     * @return
     */
    public List<BodyDataDto>  findBodyDataListByOpenid(@Param("email") String email){

        List<BodyData> bodyDataList = bodyDataService.findBodyDataList(email);
        ...
        try {
            ...
            log.error("查询身体数据历史记录={}",email+":"+e.getMessage());
            throw e;
        }
