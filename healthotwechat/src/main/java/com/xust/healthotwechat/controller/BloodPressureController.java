...
public class BloodPressureController {
    ...
    public String entryBloodPressure(@Valid BloodPressureForm bloodPressureForm,
        ...
        try {
            ...
            User user = userFacadeService.findUserByEmail(sessionEmail);

            bloodPressureForm.setEmail(sessionEmail);

            //血压值不正常是提醒
            if (Integer.parseInt(bloodPressureForm.getHighPressure())>=150){

                SmsUtils.sendMessage_(user.getEmail(), user, bloodPressureForm);
            ...
            log.error("录入血压={}",bloodPressureForm.getEmail()+e.getMessage());
    ...
    public ResultVO<List<BloodPressureDto>> getData(HttpServletRequest request){
        ...
        try {
            ...
            historyList = bloodPressureFacadeService.findBloodPressureList(email);
     ...
     */
    @PostMapping("/custody")
    public ResultVO<List<BloodPressureDto>> custodyHistory(@RequestParam("email") String email,
