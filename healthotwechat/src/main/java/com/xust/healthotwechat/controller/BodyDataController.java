...
public class BodyDataController {
    ...
    public String  entry(@Valid BodyDataForm bodyDataForm,
        ...
        try {
            ...
            bodyDataForm.setEmail(email);

            bodyDataFacadeService.entry(bodyDataForm);
            resultVo = AjaxResultVOUtils.success("录入成功");

        }catch (Exception e){
            log.error("身体数据录入异常={}",bodyDataForm.getEmail()+e.getMessage());
    ...
    public ResultVO<List<BodyDataDto>> history(HttpServletRequest request){
        ...
        try {
            ...
            String email = (String) request.getSession().getAttribute("user");
            /**查询历史记录*/
            historyList = bodyDataFacadeService.findBodyDataListByOpenid(email);
     ...
     */
    @PostMapping("/custody")
    public ResultVO<List<BodyDataDto>> custodyHistory(@RequestParam("email") String email,
