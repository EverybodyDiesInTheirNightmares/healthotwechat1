...
public class SleepingController {
    ...
    public String entry(@Valid SleepingForm sleepingForm,
        ...
        try {
            ...
            log.error("睡眠数据录入异常={}",sleepingForm.getEmail()+e.getMessage());
    ...
    public ResultVO<List<SleepingDto>> history(HttpServletRequest request){
        ...
        try {
            ...
            String email = (String) request.getSession().getAttribute("user");
            historyList = sleepingFacadeService.findSleepingListByOpenid(email);
     ...
     */
    @PostMapping("/custody")
    public ResultVO<List<SleepingDto>> custodyHistory(@RequestParam("email") String email,
