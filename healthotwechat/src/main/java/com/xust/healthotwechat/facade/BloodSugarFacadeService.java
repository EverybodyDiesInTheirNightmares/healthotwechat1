...
public class BloodSugarFacadeService {
    ...
    public int entryBloodSugar(BloodSugarForm bloodSugarForm){
        ...
        try {
            String key = bloodSugarForm.getEmail()+"_bloodSugar";
            ...
            /**redis中加入标记表示三天内录入过*/
            redisTemplate.opsForValue().set(bloodSugarForm.getEmail()+"_three_entry_bloodSugar",
     ...
     * @return
     */
    public List<BloodSugarDto> findBloodSugarListByOpenid(String email){
        List<BloodSugarDto> historyList;

        try {

            /**查询历史记录*/
            List<BloodSugar> bloodSugarList = bloodSugarService.findBloodSugarList(email);
