...
public class SleepingFacadeService {
    ...
    public int entry(SleepingForm sleepingForm){
        ...
        try{
            ...
            String key = sleepingForm.getEmail()+"sleeping";
            ...
            /**redis中加入标记表示三天内录入过*/
            redisTemplate.opsForValue().set(sleepingForm.getEmail()+"_three_entry_sleeping",
     ...
     * @return
     */
    public List<SleepingDto> findSleepingListByOpenid(@Param("email")String email){

        List<SleepingDto> histotyList;

        try {
            /**查询历史记录*/
            List<Sleeping> sleepingList = sleepingService.findSleepingList(email);
            ...
            log.error("查询睡眠历史数据={}",email+":"+e.getMessage());
            throw e;
        }
        return histotyList;
    }
